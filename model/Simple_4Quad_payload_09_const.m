...
%% AMFC - links
QL = 1e-0*eye(n);
gamma_1L = 1e+2*diag([1e-6*ones(3,1);1e-0*ones(3,1)]);
rho_1L = 1e-0;
...
Qv = 1e-0*eye(n);
gamma_1v = 1e+2*diag([1e-6*ones(3,1);1e-6*ones(3,1)]);
rho_1v = 1e-0;
gamma_0v = 1e-0*diag([1e-6*ones(3,1);1e-6*ones(3,1)]); 
rho_0v = 1e-0;

%% AMFC - Payload-position
Qo = 1e-0*eye(n);
gamma_1o = 1e+2*diag([1e-3*ones(3,1);1e-3*ones(3,1)]);
rho_1o = 1e-0;
gamma_0o = 1e-0*diag([1e-3*ones(3,1);1e-3*ones(3,1)]); 
rho_0o = 1e-0;
